...
# Author: Fatehjot Gogia
# Inspired by concepts from Computer Networking: A Top-Down Approach (8th Edition)
# ICMP type/code references: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml
...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        __errors = {"0": "echo-reply (destination reached)",                #Source: https://www.w3schools.com/python/python_dictionaries_nested.asp
                    "3": {"0": "Net was unreachable",                       #Souece: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml
                          "1": "Host was unreachable",
                          "2": "Protocol was unreachable",
                          "3": "Port was unreachable",
                          "4": "Fragmentation was needed but don't fragment flag was set",
                          "5": "Source route failed",
                          "6": "Destination netowrk is unknown",
                          "7": "Destination host is unknown",
                          "8": "Source host is isolated",
                          "9": "Communication with destination network administratively prohibted",
                          "10": "Communication with destination host administratively prohibited",
                          "11": "Destination network unreachable for type or sercice",
                          "12": "Desination host unreachable for type of sercice",
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            if self.getPacketSequenceNumber() != icmpReplyPacket.getIcmpSequenceNumber():
                validationPassed = False
                validSequenceNumber = False
            if self.getDataRaw() != icmpReplyPacket.getIcmpData():
                validationPassed = False
                validRawData = False
            
            #Debugging
            if self.__DEBUG_IcmpPacket == True:
                print(f"Expected Identifer Value: {self.getPacketIdentifier()}, Actual Identifier Value: {icmpReplyPacket.getIcmpIdentifier()}, Comparison Result (Valid Identifier): {validIdentifier}")
                print(f"Expected Sequence Number: {self.getPacketSequenceNumber()}, Actual Sequence Number: {icmpReplyPacket.getIcmpSequenceNumber()}, Comparison Result (Valid Sequence Number): {validSequenceNumber}")
                print(f"Expected Raw Data: {self.getDataRaw()}, Actual Raw Data: {icmpReplyPacket.getIcmpData()}, Comparison Result (Valid Raw Data): {validRawData}")
            
            
            
            icmpReplyPacket.setIsValidResponse(validationPassed)
            icmpReplyPacket.setIcmpIdentifier_isValid(validIdentifier)
            icmpReplyPacket.setIcmpSequenceNumber_isValid(validSequenceNumber)
            icmpReplyPacket.setIcmpRawData_isValid(validRawData)

            #Update expected Values
            icmpReplyPacket.setExpectedData(self.getDataRaw())
            icmpReplyPacket.setExpectedSequenceNumber(self.getPacketSequenceNumber())
            icmpReplyPacket.setExpectedIdentifier(self.getPacketIdentifier())
        ...
        def sendEchoRequest(self):
            ...
            try:
                ...
                if timeLeft <= 0:
                    ...
                    if icmpType == 11:
                        print(f"\nError: {self.__errors[str(icmpType)][str(icmpCode)]}\n")                          # Time Exceeded
                        ...
                        return icmpProfile                                                          #Source: https://discuss.python.org/t/returning-a-dictionary/56165
                        ...
                        timeSent = icmpReplyPacket.printResultToConsole(self.getTtl(), timeReceived, addr)
                        icmpProfile = {
                            "icmpType": icmpType,
                            "icmpCode": icmpCode,
                            "rtt": (timeReceived - timeSent) * 1000
                        }
                        return icmpProfile      # Echo reply is the end and therefore should return
    ...
    class IcmpPacket_EchoReply:
        ...
        __IcmpExpectedRawData = ''
        __IcmpExpectedSequenceNumber = 0
        __IcmpExpectedIdentifier = 0
        __errors = {"0": {"0": "echo-reply (destination reached)"},
                    "3": {"0": "Net was unreachable",
                          "1": "Host was unreachable",
                          "2": "Protocol was unreachable",
                          "3": "Port was unreachable",
                          "4": "Fragmentation was needed but don't fragment flag was set",
        ...
        def getExpectedIcmpData(self):
            return self.__IcmpExpectedRawData
        
        def getExpectedSequenceNumber(self):
            return self.__IcmpExpectedSequenceNumber
        
        def getExpectedIdentifier(self):
            return self.__IcmpExpectedIdentifier
        ...
        def setIcmpIdentifier_isValid(self, boolean):
            self.__IcmpIdentifier_isValid = boolean
        
        def setIcmpSequenceNumber_isValid(self, boolean):
            self.__IcmpSequenceNumber_isValid = boolean
        
        def setIcmpRawData_isValid(self, boolean):
            self.__IcmpRawData_isValid = boolean

        def setExpectedData(self, rawData):
            self.__IcmpExpectedRawData = rawData
        
        def setExpectedSequenceNumber(self, seqNumber):
            self.__IcmpExpectedSequenceNumber = seqNumber
        
        def setExpectedIdentifier(self, identifier ):
            self.__IcmpExpectedIdentifier = identifier
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            ...
            print(f"\nSuccess: {self.__errors[str(self.getIcmpType())][str(self.getIcmpCode())]}")   #Source: https://www.geeksforgeeks.org/python/accessing-value-inside-python-nested-dictionaries/
            ...
            if not self.isValidResponse():
                if not self.getIcmpIdentifier_isValid():
                    print(f"ERROR!: Mismatch in identifier values. Expected ({self.getExpectedIdentifier()}), Received ({self.getIcmpIdentifier()})")
                if not self.getIcmpSequenceNumber_isValid():
                    print(f"ERROR!: Mismatch in Sequence Number values. Expected ({self.getExpectedSequenceNumber()}), Received ({self.getIcmpSequenceNumber()})")
                if not self.getIcmpRawData_isValid():
                    print(f"ERROR!: Mismatch in Raw Data. Expected ({self.getExpectedIcmpData()}), Received ({self.getIcmpData()})")
            return timeSent
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            icmpProfile = icmpPacket.sendEchoRequest()                                                # Build IP
            sentPackets +=1
            if icmpProfile:
                rtts.append(round(icmpProfile["rtt"]))
                receivedPackets +=1
        ...
        #Statistics
        if  rtts:       #Fancy print statements
            print(f"Minimum RTT: {min(rtts)}\nMaximum RTT: {max(rtts)}\nAverage RTT: {sum(rtts) / len(rtts)}\nPacket Loss: {(1-(receivedPackets/sentPackets))*100}%")
        else:
            print("No valid rtts recorded")
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        ttl = 1
        icmpTypeReceived = 1
        while icmpTypeReceived != 0 and icmpTypeReceived != 3 and ttl <=50:         #conditions must be met to keep going
            sequenceNumber = ttl
            randomPacketIdentifier = (os.getpid() & 0xffff) 
            packetToSend = IcmpHelperLibrary.IcmpPacket()
            packetToSend.setTtl(ttl)
            packetToSend.buildPacket_echoRequest(randomPacketIdentifier, sequenceNumber)
            packetToSend.setIcmpTarget(host)
            icmpProfile = packetToSend.sendEchoRequest()
            if icmpProfile:
                icmpTypeReceived = icmpProfile["icmpType"]          
            if icmpTypeReceived == 0:               
                break
            else:
                ttl +=1
...
def main():
    ...
    # icmpHelperPing.traceRoute("192.168.0.0")
